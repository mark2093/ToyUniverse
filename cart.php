...
class Cart {
    ...
    public function insert($item = array()){
        if(!is_array($item) OR count($item) === 0){
            ...
            if(!isset($item['Id'], $item['Title'], $item['Points'], $item['qty'])){
                return FALSE;
            }else{
                /*
                 * Insert Item
    ...
    public function update($item = array()){
        if (!is_array($item) OR count($item) === 0){
            ...
            if (!isset($item['rowid'], $this->cart_contents[$item['rowid']])){
                ...
                foreach(array_diff($keys, array('Id', 'Title')) as $key){
    ...
    protected function save_cart(){
        ...
        foreach ($this->cart_contents as $key => $val){
            ...
            if(!is_array($val) OR !isset($val['Points'], $val['qty'])){
                continue;
            }

            $this->cart_contents['cart_total'] += ($val['Points'] * $val['qty']);
            $this->cart_contents['total_items'] += $val['qty'];
            $this->cart_contents[$key]['subtotal'] = ($this->cart_contents[$key]['Points'] * $this->cart_contents[$key]['qty']);
        }
